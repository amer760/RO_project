...
def generate_random_graph(num_vertices, probability):
    ...
    # Connect all vertices to ensure the graph is connected
    ...
    for node in sorted_nodes:
        if node not in colors:
            ...
            for other_node in sorted_nodes:
                if other_node not in colors and not any(
                    graph.has_edge(other_node, n) for n in colors if colors[n] == current_color):
...
image_path = "logo emsi.png"  # Replace with the exact file name and path
if os.path.exists(image_path):
    try:
        # Resize the image
        img = Image.open(image_path)
        img = img.resize((250, 50))  # Resize to smaller dimensions
        photo = ImageTk.PhotoImage(img)
        
        root.photo = photo  # Persist the image
        image_label = tk.Label(root, image=photo)
        image_label.image = photo  # Persist image in the label
        image_label.place(x=1, y=1)  # Position at the top-left corner
    except Exception as e:
        print(f"Error loading image: {e}")
else:
    print(f"Image file not found: {os.path.abspath(image_path)}")
...
ModernButton(buttons_frame, text="Commencer", width=20, command=show_second_interface).grid(row=20, column=0, padx=20 , pady=30)
ModernButton(buttons_frame, text="Quitter", width=20, command=root.quit).grid(row=20, column=1, padx=20, pady=30)

# Footer
footer_label = tk.Label(
    main_frame,
    text="© 2025 EMSI - Amer Bouakl et Aymane Jebrane\nEncadrée par: El Mkhalet mouna",
    font=("Helvetica", 12),
    fg="black",
    bg=WINDOW_BG
)
